...
{
	printf("\n");
	printf("-i {dsm|stdin|none|keyboard|joystick}     specify input\n");
...
void* __setpoint_manager(__attribute__ ((unused)) void* ptr)
{
	...
	while(rc_get_state()!=EXITING){
			...
			/*case W:
				switch (setpoint.drive_mode) {
					case NOVICE:
						...
						break;
					case ADVANCED:
						setpoint.phi_dot += DRIVE_RATE_ADVANCED;
						break;
					default: break;
				}
				m_input_mode = NONE;
		...
		case KEYBOARD:
			...
			switch (inputC) {
			case 'a':
				setpoint.gamma_dot += TURN_RATE_ADVANCED * 0.2;
				break;
			case 'w':
				setpoint.phi_dot -= DRIVE_RATE_ADVANCED * 0.1;
				break;
			case 's':
				setpoint.phi_dot += DRIVE_RATE_ADVANCED * 0.1;
				break;
		...
		case JOYSTICK:
			...
			fptr = fopen("/home/debian/Lab5Server/joystick.txt", "r");
			fgets(n, 150, fptr);
			printf("%s \n", n);
			fclose(fptr);
			j = 0;
			jstickh = strtod(n + 9, &ptstart);
			jstickv = strtod(ptstart + 10, &ptend);
			knob1 = strtod(ptend + 7, &ptstart);
			knob2 = strtod(ptstart + 7, &ptend);
			knob3 = strtod(ptend+7, &ptstart);
			knob4 = strtod(ptstart + 7, &ptend);
			button = strtod(ptend + 9, &ptstart);
			...
			printf("\n %f \n", button);
			//printf("\n %f \n", knob2);
			setpoint.phi_dot = DRIVE_RATE_NOVICE * jstickh*1.0/500;
			setpoint.gamma_dot = TURN_RATE_NOVICE * jstickv*1.0 /500;

			/*
			//PID CALIBRATION UNCOMMENT
			D3_KP = knob1;
			D3_KI = knob2;
			D3_KD = knob3;
			//*************
		...
		case A:
			switch (setpoint.drive_mode) {
			case NOVICE:
				setpoint.gamma_dot -= TURN_RATE_NOVICE;
				break;
			case ADVANCED:
				setpoint.gamma_dot -= TURN_RATE_ADVANCED;
				break;
			default: break;
			}
			m_input_mode = NONE;
